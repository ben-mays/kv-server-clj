...
(deftest predicate-tests
  (testing "SetEntry Predicates"
    (testing "Complex predicate happy case"
      (init-test)
      (def predicates [(pred-equals? store "1" "A") (pred-exists? store "2")]) ;; only update if "1" has value "A" and key "2" exists
    ...
    (testing "equals happy case"
      (init-test)
      (.setEntry store "1" "A")
      (is (nil? (:errors (.setEntry store "2" "B" [(pred-equals? store "1" "A")]))))
      (is (= (:value (.getEntry store "1")) "A"))
      (is (= (:value (.getEntry store "2")) "B")))

    (testing "equals failure case" 
      (init-test)
      (is (not (nil? (:errors (.setEntry store "2" "B" [(pred-equals? store "1" "A")])))))
      (is (nil? (:value (.getEntry store "2")))))
    
    (testing "exists happy case"
      (init-test)
      (.setEntry store "1" "A")
      (is (nil? (:errors (.setEntry store "2" "B" [(pred-exists? store "1")]))))
      (is (= (:value (.getEntry store "2")) "B")))
    
    (testing "exists failure case"
      (init-test)
      (is (not (nil? (:errors (.setEntry store "2" "B" [(pred-exists? store "2")])))))
      (is (nil? (:value (.getEntry store "2")))))))
...
(deftest predicate-read 
  (testing "Version Didn't Change"
    ...
    (is (= (:value (.getEntry store "my-key" [(pred-equals? store "my-key-version" 1)])) "a"))
    ...
    ;; Now client C watns update the key with key + 1 iff the key-version hasn't changed. This should fail because client b has updated my-key.
    (is (nil? (:value (.getEntry store "my-key-version" [(pred-equals? store "my-key-version" 1)]))))))
